...
public class Withdraw implements CommandExecutor {
	...
	public Withdraw(final JavaPlugin plugin) {
		this.plugin = plugin;
	}
			...
			public void run() {
				if (sender instanceof Player) {
					...
					try {
						...
						if (amount <= 0) {
							player.sendMessage("Amount has to be greater than 0 BAN");
							return;
						}
						...
						if (args.length == 2) {
							...
							player.spigot().sendMessage((new ComponentBuilder("You have sent ").color(ChatColor.YELLOW).append(amountStr + " BAN").color(ChatColor.WHITE).bold(true).append(" to ").color(ChatColor.YELLOW)
